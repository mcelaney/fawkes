...
<p>This talk will also provide context about crypto-currency and how the protocol is implemented - so that the audience has appropriate context for understanding building an exchange.</p>\n"
    ...
    test "fetch/0 returns the schedule in order with preloads", %{
      ...
      slot_talk_1 = Enum.find(slot_2_result.talks, fn slot_talk -> slot_talk.id == talk_1.id end)
      slot_talk_2 = Enum.find(slot_2_result.talks, fn slot_talk -> slot_talk.id == talk_2.id end)
      slot_talk_3 = Enum.find(slot_2_result.talks, fn slot_talk -> slot_talk.id == talk_3.id end)
    ...
    test "fetch_by_audience/1 returns the schedule in order (by start time) with preloads", %{
      ...
      [%{talks: [talk_result | []]} = slot_result | []] = Schedule.fetch_by_audience("beginner")
    ...
    test "fetch_by_category/1 returns the schedule in order (by start time) with preloads", %{
      ...
      [%{talks: [talk_result | []]} = slot_result | []] = Schedule.fetch_by_category("nerves")
  ...
  defp image_info(filename) do
    %Plug.Upload{
      ...
      filename: filename,
      path: Path.join(:code.priv_dir(:fawkes), "/static/images/" <> filename)
    }
  end
end
